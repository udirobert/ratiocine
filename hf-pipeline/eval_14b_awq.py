"""
Modal evaluation script — test the 14B-AWQ + CoT prompts against Linguini.

This loads the model from HuggingFace (AWQ, no extra quantization needed)
and scores it against the 160 Linguini problems using the CoT prompts.

Usage:
    modal run hf-pipeline/eval_14b_awq.py
    modal run hf-pipeline/eval_14b_awq.py --limit 20
    modal run hf-pipeline/eval_14b_awq.py --task-type match_letters
"""
...
app = modal.App("ratiocine-eval-14b")
...
def evaluate(
    ...
    """Evaluate the 14B-AWQ model with CoT prompts against Linguini."""
    import ast
    import os
    import sys
    import time
    ...
    # --- Load model (AWQ, no bitsandbytes needed) ---
    ...
    for i, problem in enumerate(problems):
        ...
        try:
            ...
            pred_answers = parse_answers(generated, n_expected=n_expected, task_type=tt)

...
The model outputs answers directly (no CoT), one per line. This is fast
(~5-8s/problem on T4) and preserves EM accuracy.
...
def parse_answers(text: str, n_expected: int = 0, task_type: str = "") -> list[str]:
    ...
    4. All non-empty lines (last resort)
    ...
    # 1. Try <answers> tags first (works if model uses tag format)
...
def extract_analysis(text: str) -> str:
    ...
    For direct-prompt mode, the model doesn't produce <analysis> tags,
    so we return a brief note about the approach.
    """
